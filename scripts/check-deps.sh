...
function activePortFiles() {
  ...
  switch (process.platform) {
    ...
    case 'win32':
      return [
        'D:/Playwright-Chrome-Profile/DevToolsActivePort',
...
" 2>/dev/null); then
  echo "chrome: not connected — 请用以下方式启动 Chrome："
  echo "  运行（不需要关闭日常 Chrome）:"
  echo "  Start-Process 'C:\Program Files\Google\Chrome\Application\chrome.exe' \`"
  echo "    -ArgumentList '--remote-debugging-port=9222','--user-data-dir=D:\Playwright-Chrome-Profile'"
  exit 1
fi
